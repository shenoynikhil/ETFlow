...
class TorchMDDynamicsWithEnergy(nn.Module):
    """
    TorchMDDynamics Model for DDPM training.
    ...
    def __init__(
        ...
        self.vector_output_model = EquivariantVectorOutput(
            hidden_channels=hidden_channels, activation=activation, reduce_op=reduce_op
        )
        self.scalar_output_model = Scalar(
            hidden_channels=hidden_channels, activation=activation
        )
        self.reset_parameters()

    def reset_parameters(self):
        self.representation_model.reset_parameters()
        self.vector_output_model.reset_parameters()
        self.scalar_output_model.reset_parameters()
    ...
    def forward(
        ...
        # compute energy
        _, vector = self.vector_output_model.pre_reduce(x, v, z, pos, batch)
        energy = self.scalar_output_model.reduce(
            self.scalar_output_model.pre_reduce(x, v, z, pos, batch),  # unpooled energy
            batch,
        ).view(-1, 1)
        return energy, center(vector - pos, batch)


class TorchMDDynamicsWithScore(nn.Module):
    """
    TorchMDDynamics Model for DDPM training.
    ...
    def reset_parameters(self):
        self.representation_model.reset_parameters()
        self.flow_output_model.reset_parameters()
        self.score_output_model.reset_parameters()
    ...
    def forward(
        ...
        x, v, z, pos, batch = self.representation_model(
            ...
            pos=pos,
            batch=batch,
            node_attr=node_attr.float(),  # should be float
            edge_index=edge_index,
            edge_attr=edge_attr,
        )

        # latent representation
        _, velocity = self.flow_output_model.pre_reduce(x, v, z, pos, batch)
        _, score = self.score_output_model.pre_reduce(x, v, z, pos, batch)

        velocity = center(velocity - pos, batch)
        score = center(score - pos, batch)
